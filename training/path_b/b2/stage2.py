"""Stage-2 multiclass T2D: Path A family HPO + val-select."""
...
def train_stage2(
    ...
    *,
    ...
    """Tune LGBM + CatBoost; return selected pack + raw/cal test reports."""
    ...
    log(
        f"  SELECT {selected['family']} val_auc={selected['val_macro_ovr_auc']:.4f}"
    )
...
def arm_feature_cols(
    ...
    """Resolve feature list for a pre-registered arm id."""
    w0 = groups["w0"]
    c1 = groups["c1"]
    true_cols = groups["true_cols"]
    if arm == "D0":
        return list(w0)
    if arm == "D1" or arm == "D1a":
        return list(c1)
    if arm == "D0a":
        return list(w0)
    if arm == "T0":
        return list(w0) + list(pred_cols)
    if arm == "T1":
        return list(c1) + list(pred_cols)
    if arm == "O0":
...
def is_deployable_arm(arm: str) -> bool:
    return arm in ("D0", "D1", "T0", "T1", "D0a", "D1a")
